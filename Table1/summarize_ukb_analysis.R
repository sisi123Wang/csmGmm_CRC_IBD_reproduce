...
for (file_it in 1:2) {
  ...
  tempRes <- data.frame(Method = c("New","Kernel","df50","df7","HDMT","DACT"),
                        numReject = NA_integer_)
  
  # --- New ---
  tempNew <- fread(fnameNew[file_it], header = TRUE, data.table = FALSE)
  x_new   <- tempNew[[1]]  # first column
  # align and compute rejections
  tempDat <- cleanZ %>%
    select(all_of(selections[[file_it]]), chrpos) %>%
    mutate(origIdx = row_number(),
           newLfdr = x_new) %>%
    arrange(newLfdr) %>%
...
# save summary
write.table(allResults, fnameOut, append=FALSE, quote=FALSE, row.names=FALSE, col.names=TRUE, sep='\t')
