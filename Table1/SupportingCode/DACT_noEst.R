...
#' @export
DACT_noEst = function(p_a,p_b, nullEst = NULL, correction="JC"){
  #Z_a = stats::qnorm(p_a,lower.tail = F)
  #Z_b = stats::qnorm(p_b,lower.tail = F)
  if (is.null(nullEst)) { 
    nullEst <- null_estimation(cbind(p_a, p_b))
  }
  pi0a <- nullEst$alpha1
  pi0b <- nullEst$alpha2
  ...
  #}
  if(correction == "JC"){
    correctOut <- JCCorrect(p_dact)
    p_dact <- correctOut$pval
    zStats <- correctOut$zStats 
    params <- correctOut$params
  }

  return(list(pval = p_dact))
}
