# Largely copied from HDMT package, with some changes to stop KS test from throwing
# warnings for tied p-values.
...
null_estimation <- function(input_pvalues)
{
  ...
  # get rid of ties
  for (col_it in 1:2) {
    dups <- duplicated(input_pvalues[, col_it])
    while(sum(dups) > 0) {
      input_pvalues[dups, col_it] <- input_pvalues[dups, col_it] + 10^(-7)
      dups <- duplicated(input_pvalues[, col_it])
    }
  }
